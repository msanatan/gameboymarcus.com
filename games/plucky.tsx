...
import { collides, init, initKeys, initPointer, keyPressed, Button, GameLoop, Sprite, Text } from "kontra";
import { chakra } from "@chakra-ui/react";
import { pressStart2P } from "@/app/fonts";

export default function PluckyGame() {
  ...
  useEffect(() => {
    ...
    let playButtonBlinkId: number | null = null;
    let restartButtonBlinkId: number | null = null;
    ...
    const menuTextOptions = {
      color,
      font: `20px ${pressStart2P.style.fontFamily}, sans-serif`,
      anchor: { x: 0.5, y: 0.5 },
      textAlign: "center",
    };
    ...
    const playButton = Button({
      ...
      render() {
        this.draw();
        if (this.hovered) {
          this.textNode.font = `24px ${pressStart2P.style.fontFamily}, sans-serif`;
        } else {
          this.textNode.font = `20px ${pressStart2P.style.fontFamily}, sans-serif`;
        }
      },
    });

    function buttonBlink(button: Button): void {
      button.textNode.color = button.textNode.color == color ? altColor : color;
    }

    function playButtonBlink(): void {
      buttonBlink(playButton);
    }

    function restartButtonBlink(): void {
    ...
    function isCollidingFromTop(obj1: Sprite, obj2: Sprite): boolean {
      if (obj1.dy <= 0) return false;
      const prevY = obj1.y - obj1.dy;
      return prevY + obj1.height <= obj2.y;
    }

    function isCollidingFromBottom(obj1: Sprite, obj2: Sprite): boolean {
      if (obj1.dy >= 0) return false;
      const prevY = obj1.y - obj1.dy;
      return prevY >= obj2.y + obj2.height;
    }

    function isCollidingHorizontally(obj1: Sprite, obj2: Sprite, direction: number): boolean {
    ...
    const player = Sprite({
      ...
      update(this: Sprite) {
        ...
        if ((keyPressed('arrowup') || (isMobile && jumpButton.pressed)) && !isJumping) {
    ...
    function createPlatformSegment(x: number, y: number, width: number) {
      return Sprite({
        ...platformProps,
        x,
        y,
    ...
    function createFullPlatform(y: number) {
      return createPlatformSegment(borderThickness, y, BASE_WIDTH - 2 * borderThickness);
    }

    function createPlatformWithGap(y: number) {
    ...
    const scoreText = Text({
      text: 'Score: 0',
      font: `16px ${pressStart2P.style.fontFamily}, sans-serif`,
      color: playerColor,
      x: 10,
      y: 10,
    });
    ...
    function startGame() {
      ...
      if (playButtonBlinkId) {
        ...
        playButtonBlinkId = null;
      }

      if (restartButtonBlinkId) {
        clearInterval(restartButtonBlinkId);
        restartButtonBlinkId = null;
      }
    ...
    function gameOver() {
      ...
      restartButtonBlinkId = window.setInterval(restartButtonBlink, 500);
    }
    ...
    // Start the game
    mainLoop.start();
    playButtonBlinkId = window.setInterval(playButtonBlink, 500); // Make play button blink
  }, []);

  return <chakra.canvas ref={canvasRef} width="270" height="480" />;
}
