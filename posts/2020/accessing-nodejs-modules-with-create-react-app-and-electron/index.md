...
I recently got an [Arduboy](https://arduboy.com), a hackable handheld that I can make games for. There are a lot of awesome games as well, and as an inconsistent but avid gamer, I do want to play the best ones. Unfortunately at the time, the only way for me to do so on a Mac was to clone a game from Github and compile it with the [Arduino IDE](https://www.arduino.cc/en/main/software).

I asked the Arduboy community, and it seems that some software is coming for Mac but I don't know when. I found some libraries that I could use to upload games to the Arduboy, the easiest one was [avrgirl-arduino](https://github.com/noopkat/avrgirl-arduino). Writing a script to do it via the CLI was easy thanks to this awesome library. I figured I'd make an app for it as well.
...
[Electron](https://www.electronjs.org) which is used to build the app

```console
npm i --save-dev electron
```

[Concurrently](https://github.com/kimmobrunfeldt/concurrently) and [Wait-on](https://github.com/jeffbski/wait-on) which are helper utilities we use in our scripts to develop Electron apps with a debugger.

```console
npm i --save-dev concurrently wait-on
```

[Rescripts](https://github.com/harrysolovay/rescripts) which is used to modify `create-react-app`'s webpack configuration, without ejecting the app. We need this to access Node.js libraries.
...
For those curious, I created the `isDev` function based on [this response](https://github.com/electron/electron/issues/7714#issuecomment-310885608) to a Github issue.
...
![Screenshot of running Electron app with debugger](/images/posts/electron-screenshot01.png)
...
![Screenshot of running Electron app using the fs Node.js module to list files and folders in the current directory](/images/posts/electron-screenshot02.png)
...
This got me into a pretty happy place. It's helping me build my Arduboy game uploader and I'll probably continue to use this setup for future Desktop apps I'm building with React. You can see the code for this tutorial here: [https://github.com/msanatan/react-electron-app](https://github.com/msanatan/react-electron-app).
